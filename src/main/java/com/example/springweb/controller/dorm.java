...
public class dorm {
    ...
    public String search(String dormName,Model model){
        Dorm dorm = null;
        try {
            dorm = dormService.querySingle(dormName);
        } catch (Exception e) {
        }
        model.addAttribute("dormList",dorm==null? dormService.queryAll(1).getList():dorm);
        model.addAttribute("pageInfo",dorm==null? dormService.queryAll(1) :null);
        return "dorm/list";
    }
    @GetMapping("/add")
